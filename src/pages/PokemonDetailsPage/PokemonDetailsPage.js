  ...
  return (
    <Flex justify={"center"} p={8} m={8} gap={40} fontFamily={"Flexo-Demi"}>
      <Box>
        ...
        {pokemonDetails && pokemonDetails.forms && pokemonDetails.forms.map((data)=>{
          return(<Text fontSize='3xl'  m={3} p={1}>{data.pokemonName.toUpperCase()}</Text>)
        })}
        <Box>
          {pokemonDetails &&
            pokemonDetails.type &&
      ...
      <Center>
        <Text fontSize="3xl" pt={3} pl={3} pr={3} color={'#e8e8e8'}>Poderes</Text>
        </Center>
        {pokemonDetails &&
          pokemonDetails.stats &&
          pokemonDetails.stats.map((stat) => {
            return (<>
              <Text pl={3} pb={3} mt={3} ml={5} color={'#e8e8e8'} fontSize="2xl">
                {stat.name.toUpperCase()}
                
              </Text>
              <Text pl={3} ml={5} mb={2} fontSize="2xl"> {stat.number}</Text>
              </>
            );
          })}
      ...
      <Center>
        <Text fontSize="3xl" p={3}>Principais Ataques</Text>
        </Center>
        {pokemonDetails &&
          ...
          pokemonDetails.moves.slice(0, 5).map((move) => {
            return (
              <>
                <Text p={3} m={5}  fontSize="2xl">{move.moveName.toUpperCase()}</Text>
              </>
            );
          })}
      </Box>
